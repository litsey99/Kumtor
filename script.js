...
ScrollReveal().reveal(".location__content .section__logo1", {
  ...scrollRevealOption,
  delay: 500,
});
...
ScrollReveal().reveal(".download__content .section__logo1", {
  ...scrollRevealOption,
  delay: 500,
});
...
document.getElementById("language-switch").addEventListener("change", function(event) {
  // Получаем значение выбранного языка (например, ?hl=en)
  let lang = event.target.value;

  // Перезагружаем страницу с новым параметром языка
  window.location.search = lang;
});

// Функция для загрузки контента на выбранном языке
function loadContent() {
  const urlParams = new URLSearchParams(window.location.search);
  const language = urlParams.get('hl') || 'en'; // Если язык не выбран, по умолчанию 'en'

  // Логика перевода контента
